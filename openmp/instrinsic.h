...
inline at::vec::Vectorized<scalar_t>
vec_shuffle_down(at::vec::Vectorized<scalar_t> x, size_t n) {
...
inline at::vec::Vectorized<float> vec_shuffle_down(at::vec::Vectorized<float> x,
                                                   size_t n) {
...
template <typename scalar_t>
Welford<scalar_t>
welford_vec_reduce_all(Welford<at::vec::Vectorized<scalar_t>> acc) {
  using Vec = at::vec::Vectorized<scalar_t>;
  for (size_t n = 1; n < Vec::size(); n *= 2) {
    auto shuffled =
        Welford<Vec>{vec_shuffle_down(acc.mean, n), vec_shuffle_down(acc.m2, n),
...
uint64_t randint64_cpu(uint32_t seed, uint32_t offset, int64_t low,
                       int64_t high) {
...
template <typename T>
typename std::enable_if<!std::is_reduced_floating_point<T>::value,
...
inline at::vec::Vectorized<float> masked_load(const float *src,
                                              at::vec::Vectorized<float> mask) {
  at::vec::Vectorized<float> zero_vec(0);
#if defined(CPU_CAPABILITY_AVX512)
  auto all_ones = _mm512_set1_epi32(0xFFFFFFFF);
  auto mmask =
      _mm512_cmp_epi32_mask(_mm512_castps_si512(mask), all_ones, _MM_CMPINT_EQ);
...
typename std::enable_if<
    ...
    at::vec::Vectorized<T>>::type inline masked_load(const T *src,
...
inline at::vec::Vectorized<uint8_t>
masked_load(const uint8_t *src, at::vec::Vectorized<float> mask) {
#if defined(CPU_CAPABILITY_AVX512)
  auto all_ones = _mm512_set1_epi32(0xFFFFFFFF);
  auto mmask =
      _mm512_cmp_epi32_mask(_mm512_castps_si512(mask), all_ones, _MM_CMPINT_EQ);
...
cvt_lowp_fp_to_fp32(at::vec::Vectorized<scalar_t> src) {
...
template <typename scalar_t>
inline at::vec::Vectorized<scalar_t>
cvt_fp32_to_lowp_fp(at::vec::Vectorized<float> src) {
  return at::vec::convert_from_float<scalar_t>(src, src);
}

inline at::vec::Vectorized<float>
mask_convert_to_float(at::vec::Vectorized<float> src) {
...
inline at::vec::Vectorized<float>
vec_convert_to_mask(at::vec::Vectorized<SRC> src) {
  assert(at::vec::Vectorized<float>::size() ==
         at::vec::Vectorized<SRC>::size());
...
template <>
inline at::vec::Vectorized<float>
to_float_mask(at::vec::Vectorized<float> src) {
  return src;
}
#endif
