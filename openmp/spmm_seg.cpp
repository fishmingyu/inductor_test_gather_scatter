...
extern "C" void
kernel_no_vec_seq(int feature_size, int edge_num, const long *in_ptr0,
                  const float *in_ptr1,
                  float *out_ptr0) { // in_ptr0: edge_index, in_ptr1:
                                     // input_feature, out_ptr0: output_feature
  {
    // make a bufer using std::vector
    std::vector<float> tmp_acc(feature_size, 0);
    std::vector<float> tmp0(feature_size, 0);
    int last_index = -1;
    for (long i1 = 0L; i1 < static_cast<long>(edge_num); i1 += 1L) {
      auto in_node = in_ptr0[static_cast<long>(i1)];
      // Load the current vectorized data
      for (long i0 = 0L; i0 < static_cast<long>(feature_size); i0 += 1L) {
        tmp0[i0] = in_ptr1[i0 + (feature_size * in_node)];
      }
      // Load current segment index (assuming scalar index for simplicity)
      auto current_node = in_ptr0[static_cast<long>(edge_num + i1)];
      // Check if the current index is different from the last one
      if (current_node != last_index) {
        ...
        if (last_index != -1) {
          for (long i0 = 0L; i0 < static_cast<long>(feature_size); i0 += 1L) {
            out_ptr0[last_index * feature_size + i0] = tmp_acc[i0];
          }
        }
        // Reset the accumulator and update the last index
        for (long i0 = 0L; i0 < static_cast<long>(feature_size); i0 += 1L) {
...
#pragma omp for
      for (long i = 0; i < edges; ++i) {
        if (i == 0) {
          ...
          atomic_add(&res[last_index * feature_size + j],
      ...
      atomic_add(&res[last_index * feature_size + j], acc);
    }
  }
}

torch::Tensor segment_spmm_seq(torch::Tensor edge_index,
  ...
  kernel_no_vec_seq(feature_size, edge_num, edge_index_ptr, input_feature_ptr,
                    output_feature_ptr);
...
PYBIND11_MODULE(spmm_seg, m) {
  ...
  m.def("spmm_seq", &segment_spmm_seq, "spmm_seg_seq (CPU)");
}